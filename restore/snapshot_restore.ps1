...
function Log-Message {
    ...
    Add-Content -Path ".\restore\$($task_name).log" -Value "$(Get-Date -Format "yyyy-MM-dd HH:mm:ss") - $message"
}
...
try {
    ...
    $exe_path = (Get-Item (Resolve-Path ".\restore\$($task_name).py")).FullName
...
try {
    $task = New-ScheduledTask -Action (New-ScheduledTaskAction -Execute $python_path -Argument $exe_path) -Trigger (New-ScheduledTaskTrigger -AtStartup) -Principal (New-ScheduledTaskPrincipal -UserId "SYSTEM" -LogonType ServiceAccount -RunLevel Highest)
